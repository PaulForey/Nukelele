...
package org.si.sion.module {
    ...
    //----------------------------------------
         ...
         */
        function SiOPMWavePCMData(data:*=null, samplingPitch:int=4352, srcChannelCount:int=2, channelCount:int=0)
        {
    ...
    //----------------------------------------
         ...
         */
        public function initialize(data:*, samplingPitch:int=4352, srcChannelCount:int=2, channelCount:int=0) : SiOPMWavePCMData
        {
            ...
            if (data is Sound) {
                ...
                wavelet = SiONUtil.logTransVector(data as Vector.<Number>, srcChannelCount, null, channelCount);
        ...
        public function loopTailSamples(sampleCount:int=2205, tailMargin:int=0, crossFade:Boolean=true) : SiOPMWavePCMData
        {
            ...
            if (crossFade && _loopPoint > _startPoint+sampleCount) {
                var i:int, t:Number, idx0:int, idx1:int, li0:int ,li1:int, 
                    ...
                    dt:Number = 1/imax;
                for (i=0, t=0; i<imax; i++, t+=dt) {
                    idx0 = offset + i;
                    idx1 = idx0 - imax;
                    li0 = wavelet[idx0] + envtop;
                    li1 = wavelet[idx1] + envtop;
                    wavelet[idx0] = SiOPMTable.calcLogTableIndex((log[li0] * (1-t) + log[li1] * t) * i2n);
                }
            }
